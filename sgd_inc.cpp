...
int main(int argc, const char ** argv) {
 ...
 // print_copyright();
  ...
  {
    double percent = alpha + theta * iter;
    int nshards = convert_matrixmarket_inc<EdgeDataType>(training, 0, 0, file_format, TRAINING, false, percent);
    std::vector<double> ldelta (D, 100.0);
    std::vector<double> rdelta (D, 0.0);
        ...
        for(unsigned int i = M; i < M + N; i++)
	{
	  ...
	  if(iter > 0)
	  {
	    if ((latent_factors_inmem.at(i).pvec[j] - llf_previous.at(i).pvec[j]) > rdelta.at(j))
	  	rdelta.at(j) = latent_factors_inmem.at(i).pvec[j] - llf_previous.at(i).pvec[j];
	    if ((latent_factors_inmem.at(i).pvec[j] - llf_previous.at(i).pvec[j]) < ldelta.at(j))
		ldelta.at(j) = latent_factors_inmem.at(i).pvec[j] - llf_previous.at(i).pvec[j];
	  }
	}
        ...
        {
          lbound.push_back(copy[0]);
          rbound.push_back(copy[N-1]);
        }
     }

    delete [] copy; 
    /* record delta's lowerbound and upperbound of each dimension from the 2st iteration */
...
//    std::ofstream ofs(result.c_str(), std::ofstream::out | std::ofstream::app);
    ...
    timer test_timer; 
    run_general_topn_program(pengine, &latent_factors_inmem, &llf_previous, &sgd_predict, iter, &ldelta, &rdelta, &lbound, &rbound);
    llf_previous.clear();
    for(unsigned int ele = 0; ele < M+N; ele++)
    {
	llf_previous.push_back(latent_factors_inmem.at(ele));
    }
